...
export default class DependencyGraph<T extends { toString: () => string }> {
	 ...
	 * `id`, and executes `callback` at every node (including `id`). `callback` can optionally
	 * return `false` to indicate that `callback` doesnt need to be called for its dependants.
	 */
	traverseDependantsTopologicalNew(id: T, callback: (id: T) => boolean) {
	...
	#traverseDependantsTopologicalRec(
		id: T,
		callback: (id: T) => boolean | undefined,
		...
		if (node) {
			...
			for (const dependant of node.dependants) {
				this.#traverseDependantsTopologicalRec(
					dependant,
					callback,
					visited,
					visiting,
