...
export default class DependencyGraph<T> {
	...
	/**
	 * Get the propagation order of change when `id` is the start.
	 * 
	 * @param id - The start of propagation.
	 */
	getOrder(id: T): T[] {
		const order = [] as T[];
		
		this.#getOrderRec(id, new Set(), order);

		return order;
	}

	#getOrderRec(id: T, visited: Set<T>, order: T[]) {
		if (visited.has(id)) {
			return;
		}
		else {
		...
		for (const dependant of node.dependants) {
			this.#getOrderRec(dependant, visited, order);
		}
	}

	/**
	 * Get the dependants of `id` no other node depends on.
	 * 
	 * @param id - The start of propagation.
	 */
	getLeafDependants(id: T): Set<T> {
		const leaves = new Set<T>();
		
		this.#getLeafDependantsRec(id, new Set(), leaves);

		return leaves;
	}

	#getLeafDependantsRec(id: T, visited: Set<T>, leaves: Set<T>) {
		if (visited.has(id)) {
			return;
		}
		else {
		...
		if (node.dependants.size) {
			for (const dependant of node.dependants) {
				this.#getLeafDependantsRec(dependant, visited, leaves);
			}
		}
		else {
			leaves.add(id);
